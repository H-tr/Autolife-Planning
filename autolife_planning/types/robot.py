...
class BaseConfiguration:
    ...
    def from_array(cls, array: np.ndarray | list[float]) -> "BaseConfiguration":
        if len(array) != 3:
            raise ValueError(
                f"Expected 3 elements for BaseConfiguration, got {len(array)}"
            )
...
class RobotConfiguration:
    ...
    def __post_init__(self):
        if len(self.values) != 18:
            raise ValueError(
                f"Expected 18 elements for RobotConfiguration, got {len(self.values)}"
            )
...
class ArmConfiguration:
    ...
    @classmethod
    def from_array(
        cls, array: np.ndarray | list[float], side: str
    ) -> "ArmConfiguration":
...
class WholeBodyConfiguration:
    ...
    def from_array(cls, array: np.ndarray | list[float]) -> "WholeBodyConfiguration":
        if len(array) != 21:
            raise ValueError(
                f"Expected 21 elements for WholeBodyConfiguration, got {len(array)}"
            )
    ...
    def to_array(self) -> np.ndarray:
        return np.concatenate(
            (self.robot_config.to_array(), self.base_config.to_array())
        )
