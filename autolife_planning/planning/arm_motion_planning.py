...
def plan_arm_motion(
    ...
    if start_config.side != side:
        raise ValueError(f"Start configuration side {start_config.side} does not match requested side {side}")
    if goal_config.side != side:
        raise ValueError(f"Goal configuration side {goal_config.side} does not match requested side {side}")
